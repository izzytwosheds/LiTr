...
public class BitmapOverlayFilter extends BaseOverlayGlFilter {
    ...
    /**
     * Create filter with bitmap URI, then scale, then position and then rotate the bitmap as specified.
     * @param context context for accessing bitmap
     * @param bitmapUri bitmap {@link Uri}
     * @param size size in X and Y direction, relative to video frame
     * @param topLeftPosition position of top left corner, in relative coordinate in 0 - 1 range
     *      *                 in fourth quadrant (0,0 is top left corner)
     * @param rotation counter-clockwise rotation, in degrees
     */
    public BitmapOverlayFilter(@NonNull Context context, @NonNull Uri bitmapUri, @NonNull PointF size, @NonNull PointF topLeftPosition, float rotation) {
        super(size, topLeftPosition, rotation);
        this.context = context;
        this.bitmapUri = bitmapUri;
    }

    /**
     * Create filter with client managed {@link Bitmap}, then scale, then position and then rotate the bitmap as specified.
     ...
     * @param rotation counter-clockwise rotation, in degrees
     */
    public BitmapOverlayFilter(@NonNull Bitmap bitmap, @NonNull PointF size, @NonNull PointF topLeftPosition, float rotation) {
        super(size, topLeftPosition, rotation);
        this.bitmap = bitmap;
    }
